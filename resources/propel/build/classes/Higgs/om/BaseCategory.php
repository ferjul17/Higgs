...
abstract class BaseCategory extends BaseObject implements Persistent
{
    ...
    {
        $this->subcategorysScheduledForDeletion = $this->getSubcategorys(new Criteria(), $con)->diff($subcategorys);

        foreach ($this->subcategorysScheduledForDeletion as $subcategoryRemoved) {
    ...
    public function addSubcategory(Subcategory $l)
    {
        ...
        if (!$this->collSubcategorys->contains($l)) { // only add it if the **same** object is not already associated
    ...
    public function removeSubcategory($subcategory)
    {
        if ($this->getSubcategorys()->contains($subcategory)) {
            ...
            $this->subcategorysScheduledForDeletion[]= $subcategory;
