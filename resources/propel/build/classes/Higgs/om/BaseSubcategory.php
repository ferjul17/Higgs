...
abstract class BaseSubcategory extends BaseObject implements Persistent
{
    ...
    {
        $this->subjectsScheduledForDeletion = $this->getSubjects(new Criteria(), $con)->diff($subjects);

        foreach ($this->subjectsScheduledForDeletion as $subjectRemoved) {
    ...
    public function addSubject(Subject $l)
    {
        ...
        if (!$this->collSubjects->contains($l)) { // only add it if the **same** object is not already associated
            $this->doAddSubject($l);
        }
