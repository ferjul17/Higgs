...
abstract class Forum implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Higgs\\Model\\Map\\ForumTableMap';
    ...
    /**
     * Initializes internal state of Higgs\Model\Base\Forum object.
     * @see applyDefaults()
     */
    ...
    /**
     * Compares this with another <code>Forum</code> instance.  If
     * <code>obj</code> is an instance of <code>Forum</code>, delegates to
     * <code>equals(Forum)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return Forum The current object, for fluid interface
     */
     ...
     * @return   \Higgs\Model\Forum The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[ForumTableMap::ID] = true;
        }
    ...
    public function setTitle($v)
    {
        ...
        if ($this->title !== $v) {
            $this->title = $v;
            $this->modifiedColumns[ForumTableMap::TITLE] = true;
        }
    ...
    public function setCategoryId($v)
    {
        ...
        if ($this->category_id !== $v) {
            $this->category_id = $v;
            $this->modifiedColumns[ForumTableMap::CATEGORY_ID] = true;
        }
    ...
    public function setLastPostId($v)
    {
        ...
        if ($this->last_post_id !== $v) {
            $this->last_post_id = $v;
            $this->modifiedColumns[ForumTableMap::LAST_POST_ID] = true;
        }
    ...
    public function setNbSubjects($v)
    {
        ...
        if ($this->nb_subjects !== $v) {
            $this->nb_subjects = $v;
            $this->modifiedColumns[ForumTableMap::NB_SUBJECTS] = true;
        }
    ...
    public function setNbPosts($v)
    {
        ...
        if ($this->nb_posts !== $v) {
            $this->nb_posts = $v;
            $this->modifiedColumns[ForumTableMap::NB_POSTS] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : ForumTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ForumTableMap::translateFieldName('Title', TableMap::TYPE_PHPNAME, $indexType)];
            $this->title = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ForumTableMap::translateFieldName('CategoryId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->category_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ForumTableMap::translateFieldName('LastPostId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->last_post_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : ForumTableMap::translateFieldName('NbSubjects', TableMap::TYPE_PHPNAME, $indexType)];
            $this->nb_subjects = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : ForumTableMap::translateFieldName('NbPosts', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 6; // 6 = ForumTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating \Higgs\Model\Forum object", 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildForumQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
    ...
    public function delete(ConnectionInterface $con = null)
    {
        ...
        try {
            $deleteQuery = ChildForumQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        try {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                ForumTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            $con->commit();
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ForumTableMap::ID)) {
            $modifiedColumns[':p' . $index++]  = 'ID';
        }
        if ($this->isColumnModified(ForumTableMap::TITLE)) {
            $modifiedColumns[':p' . $index++]  = 'TITLE';
        }
        if ($this->isColumnModified(ForumTableMap::CATEGORY_ID)) {
            $modifiedColumns[':p' . $index++]  = 'CATEGORY_ID';
        }
        if ($this->isColumnModified(ForumTableMap::LAST_POST_ID)) {
            $modifiedColumns[':p' . $index++]  = 'LAST_POST_ID';
        }
        if ($this->isColumnModified(ForumTableMap::NB_SUBJECTS)) {
            $modifiedColumns[':p' . $index++]  = 'NB_SUBJECTS';
        }
        if ($this->isColumnModified(ForumTableMap::NB_POSTS)) {
    ...
    {
        $pos = ForumTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $keys = ForumTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ForumTableMap::ID)) $criteria->add(ForumTableMap::ID, $this->id);
        if ($this->isColumnModified(ForumTableMap::TITLE)) $criteria->add(ForumTableMap::TITLE, $this->title);
        if ($this->isColumnModified(ForumTableMap::CATEGORY_ID)) $criteria->add(ForumTableMap::CATEGORY_ID, $this->category_id);
        if ($this->isColumnModified(ForumTableMap::LAST_POST_ID)) $criteria->add(ForumTableMap::LAST_POST_ID, $this->last_post_id);
        if ($this->isColumnModified(ForumTableMap::NB_SUBJECTS)) $criteria->add(ForumTableMap::NB_SUBJECTS, $this->nb_subjects);
        if ($this->isColumnModified(ForumTableMap::NB_POSTS)) $criteria->add(ForumTableMap::NB_POSTS, $this->nb_posts);
    ...
    {
        $criteria = new Criteria(ForumTableMap::DATABASE_NAME);
        $criteria->add(ForumTableMap::ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Higgs\Model\Forum (or compatible) type.
     ...
     *
     * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return                 \Higgs\Model\Forum Clone of current object.
     * @throws PropelException
     */
     ...
     * @throws PropelException
     */
    public function setLastPost(ChildPost $v = null)
    {
        if ($v === null) {
     ...
     * @throws PropelException
     */
    public function getLastPost(ConnectionInterface $con = null)
    {
        if ($this->aLastPost === null && ($this->last_post_id !== null)) {
            $this->aLastPost = ChildPostQuery::create()->findPk($this->last_post_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
     ...
     * If this ChildForum is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return   ChildForum The current object (for fluent API support)
     */
     ...
     * @return Collection|ChildSubject[] List of ChildSubject objects
     */
    public function getSubjectsJoinUser($criteria = null, $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildSubjectQuery::create(null, $criteria);
    ...
    {
        return (string) $this->exportTo(ForumTableMap::DEFAULT_STRING_FORMAT);
    }
