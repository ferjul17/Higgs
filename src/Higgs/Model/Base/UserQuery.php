 ...
 *
 * @method     ChildUserQuery leftJoinPostEdited($relationAlias = null) Adds a LEFT JOIN clause to the query using the PostEdited relation
 * @method     ChildUserQuery rightJoinPostEdited($relationAlias = null) Adds a RIGHT JOIN clause to the query using the PostEdited relation
 * @method     ChildUserQuery innerJoinPostEdited($relationAlias = null) Adds a INNER JOIN clause to the query using the PostEdited relation
 *
 ...
 * @method     array findBySalt(string $salt) Return ChildUser objects filtered by the salt column
...
abstract class UserQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Higgs\Model\Base\UserQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'Higgs', $modelName = '\\Higgs\\Model\\User', $modelAlias = null)
    {
    ...
    {
        if ($criteria instanceof \Higgs\Model\UserQuery) {
            return $criteria;
        }
        $query = new \Higgs\Model\UserQuery();
        if (null !== $modelAlias) {
    ...
    {
        $sql = 'SELECT ID, USERNAME, PASSWORD, SALT, EMAIL, CREATED_AT, UPDATED_AT FROM user WHERE ID = :p0';
        try {
    ...
    public function filterBySalt($salt = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(UserTableMap::SALT, $salt, $comparison);
    }
    ...
    public function filterByPost($post, $comparison = null)
    {
        if ($post instanceof \Higgs\Model\Post) {
            return $this
                ->addUsingAlias(UserTableMap::ID, $post->getUserId(), $comparison);
     ...
     * @return ChildUserQuery The current query, for fluid interface
     */
    public function filterByPostEdited($post, $comparison = null)
    {
        if ($post instanceof \Higgs\Model\Post) {
            return $this
                ->addUsingAlias(UserTableMap::ID, $post->getEditorId(), $comparison);
        } elseif ($post instanceof ObjectCollection) {
            return $this
                ->usePostEditedQuery()
                ->filterByPrimaryKeys($post->getPrimaryKeys())
            ...
            throw new PropelException('filterByPostEdited() only accepts arguments of type \Higgs\Model\Post or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the PostEdited relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return ChildUserQuery The current query, for fluid interface
     */
    public function joinPostEdited($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('PostEdited');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'PostEdited');
        }
    ...
    /**
     * Use the PostEdited relation Post object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @return   \Higgs\Model\PostQuery A secondary query class using the current class as primary query
     */
    public function usePostEditedQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinPostEdited($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'PostEdited', '\Higgs\Model\PostQuery');
    }
    ...
    public function filterBySubject($subject, $comparison = null)
    {
        if ($subject instanceof \Higgs\Model\Subject) {
            return $this
                ->addUsingAlias(UserTableMap::ID, $subject->getUserId(), $comparison);
