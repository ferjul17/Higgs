...
class PostTableMap extends TableMap
{
     ...
     */
    const CLASS_DEFAULT = 'Higgs.Model.Post';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('Id', 'Message', 'SubjectId', 'UserId', 'EditorId', 'CreatedAt', 'UpdatedAt', ),
        self::TYPE_STUDLYPHPNAME => array('id', 'message', 'subjectId', 'userId', 'editorId', 'createdAt', 'updatedAt', ),
        self::TYPE_COLNAME       => array(PostTableMap::ID, PostTableMap::MESSAGE, PostTableMap::SUBJECT_ID, PostTableMap::USER_ID, PostTableMap::EDITOR_ID, PostTableMap::CREATED_AT, PostTableMap::UPDATED_AT, ),
        self::TYPE_RAW_COLNAME   => array('ID', 'MESSAGE', 'SUBJECT_ID', 'USER_ID', 'EDITOR_ID', 'CREATED_AT', 'UPDATED_AT', ),
        self::TYPE_FIELDNAME     => array('id', 'message', 'subject_id', 'user_id', 'editor_id', 'created_at', 'updated_at', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('Id' => 0, 'Message' => 1, 'SubjectId' => 2, 'UserId' => 3, 'EditorId' => 4, 'CreatedAt' => 5, 'UpdatedAt' => 6, ),
        self::TYPE_STUDLYPHPNAME => array('id' => 0, 'message' => 1, 'subjectId' => 2, 'userId' => 3, 'editorId' => 4, 'createdAt' => 5, 'updatedAt' => 6, ),
        self::TYPE_COLNAME       => array(PostTableMap::ID => 0, PostTableMap::MESSAGE => 1, PostTableMap::SUBJECT_ID => 2, PostTableMap::USER_ID => 3, PostTableMap::EDITOR_ID => 4, PostTableMap::CREATED_AT => 5, PostTableMap::UPDATED_AT => 6, ),
        self::TYPE_RAW_COLNAME   => array('ID' => 0, 'MESSAGE' => 1, 'SUBJECT_ID' => 2, 'USER_ID' => 3, 'EDITOR_ID' => 4, 'CREATED_AT' => 5, 'UPDATED_AT' => 6, ),
        self::TYPE_FIELDNAME     => array('id' => 0, 'message' => 1, 'subject_id' => 2, 'user_id' => 3, 'editor_id' => 4, 'created_at' => 5, 'updated_at' => 6, ),
        self::TYPE_NUM           => array(0, 1, 2, 3, 4, 5, 6, )
    );
    ...
    public function buildRelations()
    {
        ...
        $this->addRelation('Forum', '\\Higgs\\Model\\Forum', RelationMap::ONE_TO_MANY, array('id' => 'last_post_id', ), null, null, 'Forums');
    } // buildRelations()
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(PostTableMap::ID);
            $criteria->addSelectColumn(PostTableMap::MESSAGE);
            $criteria->addSelectColumn(PostTableMap::SUBJECT_ID);
            $criteria->addSelectColumn(PostTableMap::USER_ID);
            $criteria->addSelectColumn(PostTableMap::EDITOR_ID);
            $criteria->addSelectColumn(PostTableMap::CREATED_AT);
            $criteria->addSelectColumn(PostTableMap::UPDATED_AT);
        } else {
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Higgs\Model\Post) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(PostTableMap::DATABASE_NAME);
            $criteria->add(PostTableMap::ID, (array) $values, Criteria::IN);
        }

        $query = PostQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) { PostTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) { PostTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(PostTableMap::ID) && $criteria->keyContainsValue(PostTableMap::ID) ) {
