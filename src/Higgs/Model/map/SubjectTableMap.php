...
class SubjectTableMap extends TableMap
{
     ...
     */
    const OM_CLASS = '\\Higgs\\Model\\Subject';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Higgs.Model.Subject';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('Id', 'Title', 'ForumId', 'UserId', 'NbPosts', 'CreatedAt', 'UpdatedAt', ),
        self::TYPE_STUDLYPHPNAME => array('id', 'title', 'forumId', 'userId', 'nbPosts', 'createdAt', 'updatedAt', ),
        self::TYPE_COLNAME       => array(SubjectTableMap::ID, SubjectTableMap::TITLE, SubjectTableMap::FORUM_ID, SubjectTableMap::USER_ID, SubjectTableMap::NB_POSTS, SubjectTableMap::CREATED_AT, SubjectTableMap::UPDATED_AT, ),
        self::TYPE_RAW_COLNAME   => array('ID', 'TITLE', 'FORUM_ID', 'USER_ID', 'NB_POSTS', 'CREATED_AT', 'UPDATED_AT', ),
        self::TYPE_FIELDNAME     => array('id', 'title', 'forum_id', 'user_id', 'nb_posts', 'created_at', 'updated_at', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('Id' => 0, 'Title' => 1, 'ForumId' => 2, 'UserId' => 3, 'NbPosts' => 4, 'CreatedAt' => 5, 'UpdatedAt' => 6, ),
        self::TYPE_STUDLYPHPNAME => array('id' => 0, 'title' => 1, 'forumId' => 2, 'userId' => 3, 'nbPosts' => 4, 'createdAt' => 5, 'updatedAt' => 6, ),
        self::TYPE_COLNAME       => array(SubjectTableMap::ID => 0, SubjectTableMap::TITLE => 1, SubjectTableMap::FORUM_ID => 2, SubjectTableMap::USER_ID => 3, SubjectTableMap::NB_POSTS => 4, SubjectTableMap::CREATED_AT => 5, SubjectTableMap::UPDATED_AT => 6, ),
        self::TYPE_RAW_COLNAME   => array('ID' => 0, 'TITLE' => 1, 'FORUM_ID' => 2, 'USER_ID' => 3, 'NB_POSTS' => 4, 'CREATED_AT' => 5, 'UPDATED_AT' => 6, ),
        self::TYPE_FIELDNAME     => array('id' => 0, 'title' => 1, 'forum_id' => 2, 'user_id' => 3, 'nb_posts' => 4, 'created_at' => 5, 'updated_at' => 6, ),
        self::TYPE_NUM           => array(0, 1, 2, 3, 4, 5, 6, )
    );
    ...
    {
        return array(
            'timestampable' => array('create_column' => 'created_at', 'update_column' => 'updated_at', 'disable_updated_at' => 'true', ),
        );
    } // getBehaviors()
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(SubjectTableMap::ID);
            $criteria->addSelectColumn(SubjectTableMap::TITLE);
            $criteria->addSelectColumn(SubjectTableMap::FORUM_ID);
            $criteria->addSelectColumn(SubjectTableMap::USER_ID);
            $criteria->addSelectColumn(SubjectTableMap::NB_POSTS);
            $criteria->addSelectColumn(SubjectTableMap::CREATED_AT);
            $criteria->addSelectColumn(SubjectTableMap::UPDATED_AT);
        } else {
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Higgs\Model\Subject) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(SubjectTableMap::DATABASE_NAME);
            $criteria->add(SubjectTableMap::ID, (array) $values, Criteria::IN);
        }

        $query = SubjectQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) { SubjectTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) { SubjectTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(SubjectTableMap::ID) && $criteria->keyContainsValue(SubjectTableMap::ID) ) {
